...
public class Waypoint : MonoBehaviour
{
    ...
    { // implement if we need to support going backwards
    ...
    public void drawPathFromHere(long currentRedrawNum)
    {
        if (lastRedrawNum < currentRedrawNum)
        {
            ...
            // draw a "finish line" if there is no next waypoint
            // just so we can see where the final waypoint is
            if (nextNum(0) == null)
            {
                Vector3 p1 = trackPtForOffset(-1.0f);
                Vector3 p2 = trackPtForOffset(1.0f);
                float siz = 4.0f;
                for (int n = 0; n < 20; n++)
                {
                    Debug.DrawLine(new Vector3(p1.x, p1.y + n*siz, p1.z), new Vector3(p2.x, p2.y + n*siz, p2.z), Color.black);
                    Debug.DrawLine(new Vector3(p1.x, p1.y + n*siz + 0.5f*siz, p1.z), new Vector3(p2.x, p2.y + n*siz + 0.5f*siz, p2.z), Color.white);
                }
            }
            ...
            for (int branch = 0; branch < next.Length; branch++)
            {
                ...
                if (nextWP == null) break; // stop looping if empty
